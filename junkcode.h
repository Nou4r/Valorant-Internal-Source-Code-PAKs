...
__declspec(noinline) void junk_rdtsc() {
    volatile auto t = __rdtsc() % 256;
    t = (t * 7 + 13) & 0xFF;
}

__declspec(noinline) void junk_qpc() {
    LARGE_INTEGER li;
    QueryPerformanceCounter(&li);
    volatile auto x = li.LowPart % 100;
    x = x ^ 0x55;
}

__declspec(noinline) void junk_math() {
    volatile float f = 1.618f;
    volatile int i = 42;
    f = f * i / 3.14f;
    i = (i << 2) ^ 0xAA;
}

__declspec(noinline) void junk_mem() {
    volatile char buf[16];
    for (int i = 0; i < 16; i++) buf[i] = (char)(i * 7);
}

// Her çağrıda rastgele junk
__forceinline void light_junk() {
    int r = (int)(__rdtsc() % 4);
    if (r == 0) junk_rdtsc();
    else if (r == 1) junk_qpc();
    else if (r == 2) junk_math();
    else junk_mem();
}
...
// Önemli fonksiyonların başına/sonuna serpiştir:
void aimbot_function() {
    light_junk();
    // ... kod ...
    light_junk();
}
